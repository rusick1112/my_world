...
def verify_mfa(request):
    if request.method == 'POST':
        ...
        if not user_id:
            messages.error(request, 'Несуществующий логин')
        ...
        if verify_2fa_otp(user, otp):
            ...
            if request.user.is_authenticated:
                messages.error(request, 'Неправильный OTP код, попоробуйте снова')
                return redirect('profile')
            messages.error(request, 'Неправильный OTP код, попоробуйте снова')
            return render(request,'otp_verify.html', {'user_id': user_id})
...
def disable_2fa(request):
    ...
    if user.mfa_enabled:
        ...
        messages.info(request, "Двухфакторная аутентификация уже отключена")
    return redirect('profile')
